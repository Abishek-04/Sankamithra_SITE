...
- **The whole price list is in the HTML.** The card grid paginates to 24, so a
  crawler used to see a quarter of the range. `PriceTable` renders all 91 rows
  as a real `<table>` on `/products`, unfiltered and unpaginated.
- **One `@graph` per page** built in `lib/schema.ts` — `Organization` and
  `WebSite` referenced by `@id`, plus `LocalBusiness` + `FAQPage` on the home
  page, `ItemList` on the catalogue, and `Product` + `VideoObject` +
  `BreadcrumbList` on each listing. Offers carry `UnitPriceSpecification` so
  the "per 1,000 packets" nuance survives into structured data.
- **`FAQ` in `lib/faq.ts` is rendered visibly *and* as schema** from the same
  source, so the two can't disagree. Items are `<details open>` — collapsed
  answers are invisible to some extraction passes, which defeats the point.
- **`public/llms.txt`** is generated by `tools/build-llms.mjs` (wired into
  `npm run build`) from the same JSON the pages use: the full price table in
  markdown, the terms, the FAQ and the channel summary.
- **`robots.ts` names the AI crawlers explicitly.** For a business that wants
  to be the cited source on Sivakasi wholesale rates, blocking them would be
  self-defeating; the list is public on the page regardless.

Titles are kept under ~60 characters and descriptions under ~160 so neither is
truncated in a result. `lang` is `en-IN`.
